...
def prepTrack(
    ...
    noPointsInterp
        Number of points in interpolated track
    noSplines
        Number of splines in interpolated track
    """
    ...
    coeffXInterp, coeffYInterp, alphaInterp, normVecNormalizedInterp, noSplines = calcSplines(
...
def interpTrack(track: npt.NDArray[np.float64], stepsize: float) -> npt.NDArray[np.float64]:
    ...
    distsCumulative = np.cumsum(np.sqrt(np.sum(np.power(np.diff(track[:, :2], axis=0), 2), axis=1)))
    distsCumulative = np.insert(distsCumulative, 0, 0.0)
...
def calcSplines(
    ...
    return xCoeffs, yCoeffs, splineAlpha, normVecNormal, noSplines
