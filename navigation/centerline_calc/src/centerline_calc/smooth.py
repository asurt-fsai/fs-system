...
K_REG: int = 3
S_REG: int = 3
STEPSIZE_PREP: float = 0.1
STEPSIZE_REG: float = 0.1
NUMBEROFSTEPS = 2000
NOPOINTSSMOOTH = 1000
...
class SmoothTrack:
    ...
    def splineApprox(self, track: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        ...
        trackInterp = self.interpTrack(track)
    ...
    def interpTrack(self, originalPath: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        ...
        noPointsInterp = math.ceil(NUMBEROFSTEPS) + 1
