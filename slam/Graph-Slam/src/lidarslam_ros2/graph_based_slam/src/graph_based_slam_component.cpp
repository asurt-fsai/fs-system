#include "../include/graph_based_slam/graph_based_slam_component.h"
#include <chrono>
...
namespace graphslam
{
  /*This is a constructor of GraphBasedSlamComponent class. It initializes the parameters and the publishers and subscribers.*/
        ...
        broadcaster_(this)
  {
    ...
    std::cout << "range_of_searching_loop_closure[m]:" << range_of_searching_loop_closure_ << std::endl;
    ...
    // set voxelgrid filter dimensions for x , y , z
    ...
    // Define a callback function for saving the map in response to a service request , returns void
               ...
               const std::shared_ptr<std_srvs::srv::Empty::Response> response) -> void
    {
      ...
      // Check if the initial map has been received.
      if (initial_map_array_received_ == false)
      {
        // If the initial map has not been received, print a message and return.
        std::cout << "initial map is not received" << std::endl;
        return;
      }

      // If the initial map is received, trigger the pose adjustment process.
      doPoseAdjustment(map_array_msg_, true);
    };
    // Create a service server for saving the map and associate the callback function with it
    map_save_srv_ = create_service<std_srvs::srv::Empty>("map_save", map_save_callback);
  }

  void GraphBasedSlamComponent::initializePubSub()
  {
    /* Initializes publishers and subscribers for the graph-based SLAM component.

     This function sets up subscriptions to receive map array updates, creates a timer to trigger
     the search loop at regular intervals, and establishes publishers for modified map, modified
     map array, and modified path data. Additionally, it logs the initialization process.
  ...
  */
  ...
  {
    /*Search for loop closures within the received map array.

      This function searches for loop closures within the received map array by comparing
      the latest submap with other submaps. It calculates the distance between submaps
      and checks predefined criteria to identify potential loop closure candidates. If
      a candidate is found, it aggregates point clouds from nearby submaps, filters the
      aggregated cloud, and performs registration to find the transformation between
      the latest submap and the candidate submap. If the fitness score of the alignment
      falls below a threshold, indicating a potential loop closure, it handles the loop
      closure by updating the loop edge data structure and performing pose adjustment
      on the map.
  ...
  */
  ...
  void GraphBasedSlamComponent::doPoseAdjustment(
      ...
      bool do_save_map)
  /* Adjusts the poses of submaps in a given map array using graph-based SLAM.

This function initializes a sparse optimizer and applies the Levenberg-Marquardt
optimization algorithm to adjust the poses of submaps while respecting constraints
derived from sensor measurements and loop closure detections.

Parameters
----------
map_array_msg : lidarslam_msgs::msg::MapArray
    The input map array containing submaps with initial poses and point cloud data.
do_save_map : bool
    Flag indicating whether to save the modified map to a PCD file.
