...
def costConfigurations(
    ...
    angleCost = calcAngleCostForConfigurations(
        pointsXY, configurations
    )
    thresholdDistance = 3 # maximum allowed distance between cones is 5 meters
    residualDistanceCost = calcDistanceCost(
        pointsXY, configurations, thresholdDistance
    )
    ...
    factors: FloatArray = np.array([1000.0, 200.0, 5000.0, 1000.0, 0.0, 1000.0, 1000.0]) #[1000.0, 200.0, 5000.0, 1000.0, 0.0, 1000.0, 1000.0]
...
def calcAngleCostForConfigurations(
    ...
    anglesAreUnderThreshold = np.logical_and(
        angles < np.deg2rad(40), isPartOfConfiguration
    )
    ...
    # get sum of costs
    costs: FloatArray = anglesAsCostFiltered.sum(
        axis=-1
    ) / isPartOfConfiguration.sum(axis=-1)
