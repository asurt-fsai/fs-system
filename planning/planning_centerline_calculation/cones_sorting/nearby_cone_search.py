...
def _implNumberOfConesOnEachSideForEachConfig(
    ...
    closeIdxs = findNearbyConesForIdxs(
        idxsInAllConfigs, distanceMatrixSquare, searchDistance
    )
...
def preCalculateSearchDirections(
    ...
    for c in configs:
        ...
        calculateMatchSearchDirectionForOneIfNotInCache(
            conesXY, keyFirst, coneType, cache
        )
        calculateMatchSearchDirectionForOneIfNotInCache(
            conesXY, keyLast, coneType, cache
        )

        for j in range(1, len(c) - 1):
            key = (c[j - 1], c[j], c[j + 1])
            calculateMatchSearchDirectionForOneIfNotInCache(
                conesXY, key, coneType, cache
            )
...
def calculateMatchSearchDirectionForOneIfNotInCache(
    ...
    if key not in cacheDict:
        cacheDict[key] = calculateSearchDirectionForOne(
            conesXY, key[0::2], coneType
        )
...
def angleBetweenSearchDirectionOfConeAndOtherConeIsTooLarge(
    ...
    goodAngle = (
        vecAngleBetween(fromConeToOtherCone, searchDirection) < searchAngle / 2
    )
    badAngle = (
        vecAngleBetween(fromConeToOtherCone, -searchDirection) < searchAngle / 2
    )
