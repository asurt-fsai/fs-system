...
def findAllEndConfigurations(
    ...
    if len(firstKIndicesMustBe) > 0 and len(endConfigurations) > 0:
        maskKeep = (
            endConfigurations[:, : len(firstKIndicesMustBe)]
            == firstKIndicesMustBe
        ).all(axis=1)
    ...
    endConfigurations[
        maskLastConeIsNotOfType, lastConeInEachConfigIdxMasked
    ] = -1
...
def implFindAllEndConfigurations(
    ...
    maskEndConfigurationsWithMoreThanTwoNodes = (
        returnValueEndConfigurations != -1
    ).sum(axis=1) > 2
    ...
    for i in range(len(canBeAdded)):
        ...
        if canBeAdded[i] and positionInStack >= 1:
            ...
            if positionInStack >= 2:
                ...
                thirdToLastToSecondToLast = (
                    secondToLastInAttempt - thirdToLast
                )
                angle3 = cast(
                    FLOAT,
                    np.arctan2(
...
def checkIfNeighborLiesBetweenLastInAttemptAndCandidate(
    ...
    for neighbor in neighbors:
        ...
        neighborToLastInAttempt = (
            trace[currentAttempt[positionInStack]] - trace[neighbor]
        )
...
def linesSegmentsIntersectIndicator(
    ...
    homogeneous = _makeSegmentsHomogeneous(
...
def _handleLineSegmentsIntersectParallelCase(
    ...
    if segmentAStart[axisToUse] < segmentBStart[axisToUse]:
        ...
        rightSegmentStartScalar = min(
            segmentBStart[axisToUse], segmentBEnd[axisToUse]
        )
    else:
        leftSegmentEndScalar = segmentBEnd[axisToUse]
        rightSegmentStartScalar = min(
            segmentAStart[axisToUse], segmentAEnd[axisToUse]
        )
