...
TEST(TestFilterLocal, TestNoPosHeading) {
  std::vector<Cone> cones = {Cone(1, 0, 0),   Cone(2, 1, 0),    Cone(2, -1, 0),
                             Cone(0.1, 1, 0), Cone(-0.1, 1, 0), Cone(5, 0, 0)};
...
TEST(TestFilterLocal, TestPosHeading) {
  ...
  TreeSearch tree_search = TreeSearch(cones, params);
  cones = tree_search.filterLocal(cones, params.field_of_view, params.distance,
                                  1.9, 0.0, M_PI);
...
TEST(TestTriangulate, Test1) {
  ...
  for (int i = 0; i < tree_search.waypoints.size(); i++) {
    EXPECT_NEAR(tree_search.waypoints[i][0], ground_truth[i][0], 0.0001);
    EXPECT_NEAR(tree_search.waypoints[i][1], ground_truth[i][1], 0.0001);
  }
}
