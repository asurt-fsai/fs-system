...
TEST(TestNearestConeToLine, TestNoCones) {
  ...
  std::array<float, 2> left_unit_normal =
      waypoint1.getLeftUnitNormal(waypoint2);
  float intercept = waypoint1.getIntercept(waypoint2);

  float dist = path.getNearestConeToLine({}, left_unit_normal, intercept,
                                         waypoint1, waypoint2);
...
TEST(TestNearestConeToLine, TestWithinBounds) {
  ...
  float dist = path.getNearestConeToLine({Cone(0, 0.5, 0), Cone(0, 1, 0)},
                                         left_unit_normal, intercept, waypoint1,
                                         waypoint2);
  EXPECT_FLOAT_EQ(dist, sqrt(1.0 / 8.0));

  float dist2 = path.getNearestConeToLine({Cone(0.5, 0, 0), Cone(1, 0, 0)},
                                          left_unit_normal, intercept,
                                          waypoint1, waypoint2);
...
TEST(TestNearestConeToLine, TestOutBounds) {
  ...
  float dist = path.getNearestConeToLine({Cone(-1, 1, 0), Cone(-1.1, 1, 0)},
                                         left_unit_normal, intercept, waypoint1,
                                         waypoint2);
  EXPECT_FLOAT_EQ(dist, sqrt(2.0));

  float dist2 = path.getNearestConeToLine({Cone(2, 3, 0), Cone(2, 3.2, 0)},
                                          left_unit_normal, intercept,
                                          waypoint1, waypoint2);
...
TEST(TestAddWaypoint, TestEmptyPath) {
  Path path;
  EXPECT_THROW(path.addWaypoint(0, 0);, std::invalid_argument);
}
...
TEST(TestColorCost, TestAllConesWrong) {
  std::vector<Cone> cones{Cone(0, 1, 1), Cone(1, 0, 0), Cone(2, 1, 1),
                          Cone(3, 0, 0)};
  Path path(cones);
  path.addWaypoint(1, 1);
  path.addWaypoint(2, 0);
...
TEST(TestColorCost, TestAllConesCorrect) {
  std::vector<Cone> cones{Cone(0, 1, 0), Cone(1, 0, 1), Cone(2, 1, 0),
                          Cone(3, 0, 1)};
  Path path(cones);
  path.addWaypoint(1, 1);
  path.addWaypoint(2, 0);
...
TEST(TestCreateCopy, TestCopy) {
  ...
  Path new_path = path.createCopy();
  EXPECT_EQ(path.waypoints.size(), new_path.waypoints.size());
  EXPECT_TRUE(path.waypoints[0] == new_path.waypoints[0]);
  EXPECT_TRUE(path.waypoints[1] == new_path.waypoints[1]);
  EXPECT_TRUE(path.waypoints[2] == new_path.waypoints[2]);
  EXPECT_TRUE(path.left_dists == new_path.left_dists);
  EXPECT_TRUE(path.right_dists == new_path.right_dists);
  EXPECT_EQ(path.cones.size(), new_path.cones.size());
  for (int i = 0; i < new_path.cones.size() && i < path.cones.size(); i++) {
    EXPECT_TRUE(path.cones[i].x == new_path.cones[i].x);
    EXPECT_TRUE(path.cones[i].y == new_path.cones[i].y);
  }
  EXPECT_TRUE(path.undet_left == new_path.undet_left);
  EXPECT_TRUE(path.undet_right == new_path.undet_right);
  EXPECT_TRUE(path.left_dists == new_path.left_dists);
  EXPECT_TRUE(path.right_dists == new_path.right_dists);
  EXPECT_TRUE(path.left_unit_normals == new_path.left_unit_normals);
}
