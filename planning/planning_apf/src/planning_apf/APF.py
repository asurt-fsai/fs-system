...
class APF:  # pylint: disable=too-many-instance-attributes
    ...
    def distanceToObstacles(self, obstacles: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        Computes the distance to the obstacles.
        """
        distances = [np.linalg.norm(cone - self.currentPosition) for cone in obstacles]
        sortedCones = [cone for _, cone in sorted(zip(distances, obstacles))]
        return np.array(sortedCones)
    ...
    def newAttractive(self) -> npt.NDArray[np.float128]:
        ...
        inYellow = nearestCone1 in self.obsYellow and nearestCone2 in self.obsYellow
        inBlue = nearestCone1 in self.obsBlue and nearestCone2 in self.obsBlue
        if inYellow or inBlue:
    ...
    def pathPlanPlot(self) -> None:  # pylint: disable=too-many-branches
        """
        Plots the path plan.
        """
