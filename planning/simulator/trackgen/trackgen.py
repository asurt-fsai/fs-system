...
class Track(object):
    ...
    def plot(self, cones=False, aveDist=3.0, show=False, filename="track.png"):
        """
    ...
    def compTrackXY(self):
        ...
        for jj in range(0, nseg):
            if self.crns[jj]:
                phi = np.linspace(0.0, self.delTh[jj], nplot)
    ...
    def populateCones(self, aveDist):
        ...
        for jj in range(0, nseg):
            if self.crns[jj]:
                ...
                n1 = (
                    int(ceil(self.delTh[jj] * abs(r1) / aveDist)) + 1
                )  # number of points used on left boundary
                n2 = (
                    int(ceil(self.delTh[jj] * abs(r2) / aveDist)) + 1
                )  # number of points used on right boundary

                phi1 = np.linspace(0.0, self.delTh[jj], n1)
                phi2 = np.linspace(0.0, self.delTh[jj], n2)
...
def compCurvature(lpar, delTh, lmax, dthmax):
    ...
    curv = (np.linalg.norm(lpar) / lmax) ** 2 + (np.linalg.norm(delTh) / dthmax) ** 2
