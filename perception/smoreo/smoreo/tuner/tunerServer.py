...
class Tuner:
    ...
    def parametersCallback(self, config: smoreoConfig, level: Any) -> smoreoConfig:
        ...
        self.node.declare_parameter("smoreo/f", config["f"])
        self.node.declare_parameter("smoreo/cx", config["cx"])
        self.node.declare_parameter("smoreo/cy", config["cy"])
        self.node.declare_parameter("smoreo/cone_height", config["cone_h"])
        self.node.declare_parameter("smoreo/camera_height_from_ground", config["camera_h"])
        self.node.declare_parameter("smoreo/cut_off_y", config["cut_off_y"])
        return config
    ...
    def start(self) -> None:
        ...
        Start the node
        """
        if not self.node.has_parameter("/smoreo/camera_raw") or not self.node.has_parameter("/smoreo/cut_off_viz"):
            raise ValueError(
        ...
        self.cutOffPublisher = self.node.create_publisher(Image,
            self.node.get_parameter("/smoreo/cut_off_viz"), queue_size=10
        )
        self.bridge = CvBridge()
        self.lastImage = None
        self.subscription = self.node.create_subscription(Image,self.node.get_parameter("/smoreo/camera_raw"), self.imageCallback)
