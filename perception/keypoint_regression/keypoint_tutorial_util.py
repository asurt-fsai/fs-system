"""This file has the utility functions for the keypoint tutorial"""
# mypy: ignore-errors
# pylint: disable=all

import torch

from utils import calculate_distance, calculate_mean_distance
...
def eval_model(model, dataloader, loss_function, input_size):
    ...
    with torch.no_grad():
        ...
        for x_batch, y_hm_batch, y_point_batch, image_name, _ in dataloader:
            ...
            loc_loss, geo_loss, loss = loss_function(
                output[0], output[1], y_hm_batch, y_point_batch
            )
    ...
    print(
        f"\tValidation: MSE/Geometric/Total Loss: {round(val_loc_loss,10)}/{round(val_geo_loss,10)}/{round(val_loss,10)}"
    )
...
def print_kpt_L2_distance(model, dataloader, kpt_keys, study_name, evaluate_mode, input_size):
    ...
    for x_batch, y_hm_batch, y_point_batch, _, image_shape in dataloader:
        ...
        if evaluate_mode:
            ...
            validation_textfile.write(
                f"{[width.numpy()[0],height.numpy()[0]]}:{single_img_kpt_dis_sum}\n"
            )
        ###########################################################################
